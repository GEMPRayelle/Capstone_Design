...
public class ListenerManager
{
    ...
    {
        GameObject cursor = Managers.Resource.Load<GameObject>("Cursor");

        spawnController = cursor.GetComponent<SpawnController>();
        mouseController = cursor.GetComponent<MouseController>();
    }
    ...
    private void SetBasicListenerResponse(GameEventListener listener, string listenerName)
    {
        ...
        switch (listenerName)
        {
            case "AllmoveFinish_Listener":
                ...
                break;
            case "AllPlayerSpawned_Listener":
                //listener.Response.AddListener(); spawnController -> MouseController의 모든 플레이어가 스폰되면 실행할 함수 등록
                break;
            // 다른 기본 이벤트 리스너 추가
            default:
    ...
    private void SetGameObjectListenerResponse(GameEventGameObjectListener listener, string listenerName)
    {
        ...
        switch (listenerName)
        {
            ...
            case "PlayerSpawned_Listener":
                listener.Response.AddListener(spawnController.InstantiatePlayerByOrder);
                break;
    ...
    private string[] GetAllListenerNames()
    {
        ...
        return new string[]
        {
            ...
            "PlayerSpawned_Listener",
            "AllPlayerSpawned_Listener"
            // 필요한 리스너들 추가
        };
